...
import { FaHandshakeAngle, FaLocationDot, FaLocationPin, FaLocationPinLock, FaPhone } from "react-icons/fa6";
    ...
    <div className="min-h-[80vh] flex flex-col justify-center px-6 sm:px-12 lg:px-20 xl:px-32 my-28">
      ...
      >
        <h2 className="text-orange-300/90 text-xl sm:text-2xl lg:text-3xl font-light tracking-wide">
          {jobTitle}<span className="text-white/90 font-bold"> Developer</span>
        </h2>
      </div>
...
>
  ...
  <span className="text-sm font-semibold text-black group-hover:text-black">
    Connect
  </span>
</a>
      ...
      >
         <div className="flex flex-wrap gap-4 mb-6">
          {/* <div  className={`group cursor-pointer relative w-fit h-fit flex items-center gap-2 p-4 bg-gray-900/30 backdrop-blur-sm rounded-lg border border-gray-800 transition-all duration-300 hover:border-orange-500/50 hover:bg-gray-800/50 hover:shadow-lg hover:shadow-orange-500/20 transform hover:-translate-y-2 hover:scale-110`}>
            <FaPhone className={`text-lg text-gray-400 group-hover:text-orange-300 transition-colors duration-300 hover:text-orange-300`} />
            <span className="text-sm font-bold my-auto sm:text-base text-orange-300">{phone}</span>
          </div> */}
          <div  className={`group relative w-fit h-fit flex items-start gap-2 py-0 sm:py-1 md:py-2 transition-all duration-300 `}>
            <FaLocationDot  className={`text-2xl text-orange-500 group-hover:text-orange-300 transition-colors duration-300 hover:text-orange-300`} />
            <span className="text-sm sm:text-base my-auto text-orange-200 font-bold">{location}</span>
          </div>
        </div>
        <div
        //  className="grid w-fit grid-cols-1 sm:grid-cols-2 gap-4"
         ...
         >
          ...
          >
            <social.icon className={`text-lg text-gray-400 group-hover:text-orange-300 transition-colors duration-300 ${social.color}`} />          
            {/* Tooltip */}
            <div className={`absolute -bottom-10 left-1/2 transform -translate-x-1/2 px-2 py-1 bg-gray-800 text-xs text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap`}>
              {social.label}
            </div>
          </a>
          ))}
